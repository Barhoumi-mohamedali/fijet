...
if (!class_exists('Advanced_Post_Listing_Block')) {
    ...
    class Advanced_Post_Listing_Block  {
        ...
        public function __construct() {  $this->aplb_register_plugin_hooks();  }
        ...
        function aplb_listing_layout($args, $attributes) {
            ...
            $marginbottom = str_replace("px", "", $row_gap);
            $marginbottom .='px';
            $css =  '
            .post-container {
            ...
            if ($query->have_posts()) :
                while ($query->have_posts()) : $query->the_post();
                    ...
                    $image_url = get_the_post_thumbnail_url(get_the_ID(), $attributes['imageUrl']);
                    if(!empty($image_url)){
                    ...
                    if (isset($attributes['showContent']) && $attributes['showContent'] == 1) {
                        ...
                        $list_items_markup .= '        <a target="_blank" href="' . esc_url($post_permalink) . '" class="read-more-button">Read more</a>';
                        $list_items_markup .= '    </div>';
                        $list_items_markup .= '</div>';
